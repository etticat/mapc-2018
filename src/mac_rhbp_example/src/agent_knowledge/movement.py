...
def get_knowledge_base_tuple_facility_movement(agent_name, goal,
...
class MovementKnowledge():
    ...
    def start_movement(self, destination):
        search = get_movement_tuple_search(self.agent_name, self.behaviour_name)
        new = get_movement_tuple(self.agent_name, self.behaviour_name, active=True, lat=destination.lat, long=destination.long)

        ret_value = self.__kb_client.update(search, new, push_without_existing = True)

    def stop_movement(self):
        search = get_movement_tuple_search(self.agent_name, self.behaviour_name)
        new = get_movement_tuple(self.agent_name, self.behaviour_name, active=False)
